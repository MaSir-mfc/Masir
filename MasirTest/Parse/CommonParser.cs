...
{
    public class CommonParser : MaParserBase
    {
        ...
        public CommonParser()
        {
            ...
            /*
             <!--Ma_Common{db=MasirBrands;tb=View_V4_OA_JSJ_CompanyInfo;top=5;order=ComIndexId desc;where=(feli=43) AND (til>2);ctype=1;tmp=/vMall/temp/test.html}-->
             * db:数据库名
             * tb:表或视图名
             * top:数据条数
             * order:排序            
        ...
        public override void ParseTag(MaUrl urlInfo, IMaSite site, IMaSkin skin, MaPage page, ref StringBuilder pageCode, MaTag pTag)
        {
        ...
        public void ParseContentTag(DataTable dt, IMaSkin skin, TagEntity tag, ref StringBuilder pageCode, MaTag pTag)
        {
        ...
        public DataTable GetContentDataTable(TagEntity tag, MaTag pTag)
        {
            ...
            DataTable _dt;
            using (Masir.MaDataHelper helper = Masir.MaDataHelper.GetDataHelper(tag.Database))
            {
        ...
        public void ParseListTag(DataTable dt, IMaSkin skin, TagEntity tag, ref StringBuilder pageCode, MaTag pTag)
        {
            if (dt != null && dt.Rows.Count > 0)
            {
                string _tempCode = skin.GetTemplate(tag.Template);
        ...
        public DataTable GetListDataTable(TagEntity tag, MaTag pTag)
        {
        ...
        public void ParsePagerTag(DataTable dt, IMaSkin skin, TagEntity tag, ref StringBuilder pageCode, int totalCount, MaTag pTag)
        {
            if (string.IsNullOrEmpty(tag.Order))
            {
                throw new Exception("必须提供order参数");
        ...
        public DataTable GetPagerDataTable(TagEntity tag, MaTag pTag, out int totalCount)
        {
            if (!string.IsNullOrEmpty(pTag["m"]))
            {
                var _dataHelper = new TagDataHelper(tag, pTag);
